...
def main():
    ...
    try:
        ...
        invoice_manager = InvoiceManager(sheets_client)
        
        # Step 1: Get customers
        customers = invoice_manager.get_customers()
        
        # For testing, just select the first customer
        if customers:
            customer = customers[0]
            logger.info(f"Selected customer: {customer['name']}")
            
            # Step 2: Get tasks for the customer
            tasks = invoice_manager.get_customer_tasks(customer['name'])
            
            # For testing, select all tasks
            if tasks:
                logger.info(f"Selected {len(tasks)} tasks for invoice")
                
                # Step 3: Generate invoice
                pdf_path = invoice_manager.generate_invoice(customer, tasks)
                
                if pdf_path:
                    # Step 4: Send invoice email
                    # Extract invoice number from generated PDF filename
                    import re
                    match = re.search(r'faktura_(\d+)_', pdf_path)
                    invoice_number = int(match.group(1)) if match else 0
                    success = invoice_manager.send_invoice_email(customer['email'], pdf_path, customer['name'], invoice_number)
                    
                    if success:
                        logger.info("Invoice process completed successfully")
                    else:
                        logger.error("Failed to send invoice email")
                else:
                    logger.error("Failed to generate invoice PDF")
            else:
                logger.error("No tasks found for customer")
        else:
