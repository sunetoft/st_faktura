...
def select_tasks(tasks: List[Dict[str, str]]) -> Optional[List[Dict[str, str]]]:
    ...
    display_tasks(tasks)
    while True:
        sel = input("Select tasks (e.g. 1,3) or 'all' or 'q': ")
...
def main() -> None:
    ...
    # Step 6: Send copy to Sune
        ...
        if bookkeeping_email:
            if invoice_manager.send_invoice_email(
                bookkeeping_email, pdf_path, selected_customer['name'], invoice_num
            ):
                print(f"✅ Bookkeeping copy sent to {bookkeeping_email}")
            else:
                print(f"❌ Failed to send bookkeeping copy to {bookkeeping_email}")
