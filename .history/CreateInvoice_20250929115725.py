...
def Credit_memo() -> bool:
    """Prompt user at start whether this document is a Credit Memo.
    Returns True if user selects credit memo (Kreditnota) else False."""
    try:
        choice = input("Generate a Credit Memo (Kreditnota)? (y/N): ").strip().lower()
        return choice == 'y'
...
def main() -> None:
    ...
    print("\n" + "="*60)
    header_label = "KREDITNOTA" if credit_memo else "INVOICE CREATION"
    print(f"ST_FAKTURA - {header_label}")
    print("="*60)
