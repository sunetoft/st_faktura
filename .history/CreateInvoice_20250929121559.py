...
"""
            ...
            if auth_method == 'oauth':
                if Credentials is None:
                    logger.error("google-auth libraries not installed. Install google-auth and google-auth-oauthlib or switch EMAIL_AUTH_METHOD=password")
                    return False
                token_file = os.getenv('GMAIL_TOKEN_FILE', 'gmail_token.json')
                if not os.path.exists(token_file):
                    logger.error(f"OAuth token file '{token_file}' not found. Run initial OAuth flow to create it.")
                    return False
                try:
                    ...
                    if not access_token:
                        logger.error("Failed to obtain access token from OAuth credentials")
                        return False
                    auth_string = self._generate_xoauth2_string(sender_email, access_token)
                    code, resp = server.docmd('AUTH', 'XOAUTH2 ' + auth_string)
                    if code != 235:
                        logger.error(f"XOAUTH2 authentication failed: {code} {resp}")
                        return False
                except Exception as e:
            ...
            # Optional bookkeeping copy (silent if fails)
