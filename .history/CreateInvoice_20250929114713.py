...
# Credit Memo option helper
def Credit_memo() -> bool:
    """Prompt whether to generate a Credit Memo instead of a standard Invoice."""
...
class InvoiceManager:
    ...
    def generate_invoice(
        ...
        Args:
            ...
            credit_memo: Whether this is a credit memo
...
def main() -> None:
    ...
    try:
        ...
        # Display header with appropriate title
        ...
        pdf_path = invoice_manager.generate_invoice(
            selected_customer, selected_tasks, hourly_rate, credit_memo=credit_memo
        )
        ...
        if invoice_manager.send_invoice_email(copy_email, pdf_path, selected_customer['name'], invoice_num):
            print(f"✅ Copy sent to {copy_email}")
        else:
            print(f"❌ Failed to send copy to {copy_email}")
        # Step 7: Send copy to bookkeeping
        bookkeeping_email = os.getenv('BOOKKEEPING_EMAIL')
        if bookkeeping_email:
            if invoice_manager.send_invoice_email(bookkeeping_email, pdf_path, selected_customer['name'], invoice_num):
                print(f"✅ Copy sent to bookkeeping at {bookkeeping_email}")
            else:
                print(f"❌ Failed to send copy to bookkeeping at {bookkeeping_email}")
        print(f"\n🎉 Invoice process completed.")
