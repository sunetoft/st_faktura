...
try:
    from google.oauth2.credentials import Credentials  # For Gmail OAuth token refresh
    from google.auth.transport.requests import Request
except ImportError:  # Soft dependency; only needed for EMAIL_AUTH_METHOD=oauth
...
def Credit_memo() -> bool:
    """Prompt user whether to create a Credit Memo (Kreditnota) instead of a normal invoice."""
    choice = input("Create a Credit Memo (Kreditnota) instead of an Invoice? (y/N): ").strip().lower()
    return choice == 'y'
...
class InvoiceManager:
    ...
    def send_invoice_email(
        ...
        try:
            ...
            bookkeeping_email = os.getenv('BOOKKEEPING_EMAIL', '').strip()

            if not sender_email:
                logger.error("SENDER_EMAIL missing in environment; cannot send email")
                return False

            # Build message function (so we can reuse for bookkeeping copy)
...
"""
                ...
                part.add_header('Content-Disposition', f'attachment; filename=faktura_{invoice_number}.pdf')
                msg.attach(part)
                return msg
            ...
            for attempt in range(1, 4):
                server = None
                try:
                    logger.info(f"[Email] Attempt {attempt} connecting to {smtp_server}:{smtp_port} using {auth_method} auth")
                    server = smtplib.SMTP(smtp_server, smtp_port, timeout=30)
                    code, banner = server.noop()
                    # Some servers may reject NOOP pre-ehlo; ignore non-2xx
                except Exception:
                    # proceed anyway; server object may still be usable
                    pass
                try:
                    server.ehlo()
                    if smtp_port == 587:
                    ...
                    if auth_method == 'oauth':
                        if Credentials is None:
                            logger.error("google-auth libs missing; pip install google-auth google-auth-oauthlib or switch to password auth")
                            return False
                        token_file = os.getenv('GMAIL_TOKEN_FILE', 'gmail_token.json')
                        if not os.path.exists(token_file):
                            logger.error(f"OAuth token file '{token_file}' not found. Run initial OAuth consent flow.")
                            return False
                        ...
                        if not access_token:
                            logger.error("OAuth credentials missing access token")
                            return False
                        auth_string = self._generate_xoauth2_string(sender_email, access_token)
                        code, resp = server.docmd('AUTH', 'XOAUTH2 ' + auth_string)
                        if code != 235:
                            raise RuntimeError(f"XOAUTH2 auth failed: {code} {resp}")
                    else:
                        if not sender_password:
                            logger.error("SENDER_PASSWORD missing and EMAIL_AUTH_METHOD=password; cannot authenticate")
                            return False
                        server.login(sender_email, sender_password)

                    # Small NOOP to verify connection alive post-auth
                    try:
                        server.noop()
                    except Exception as noop_err:
                        logger.warning(f"SMTP NOOP failed post-auth (continuing): {noop_err}")

                    # Send main email
                    customer_msg = build_message(customer_email)
                    server.sendmail(sender_email, [customer_email], customer_msg.as_string())
                    logger.info(f"Invoice email sent to {customer_email}")

                    # Bookkeeping copy
                    if bookkeeping_email and bookkeeping_email.lower() != customer_email.lower():
                        try:
                            bookkeeping_msg = build_message(bookkeeping_email)
                            bookkeeping_msg['Subject'] = f"(Kopi) Faktura #{invoice_number} - {customer_name}"
                            server.sendmail(sender_email, [bookkeeping_email], bookkeeping_msg.as_string())
                            logger.info(f"Bookkeeping copy sent to {bookkeeping_email}")
                        except Exception as be:
                            logger.warning(f"Failed to send bookkeeping copy: {be}")

                    server.quit()
                    return True
                except Exception as attempt_err:
                    last_error = attempt_err
                    logger.error(f"[Email] Attempt {attempt} failed: {attempt_err}")
                    try:
                        if server:
                            server.quit()
                    except Exception:
            ...
            logger.error(f"All email attempts failed after 3 retries: {last_error}")
            return False
        except Exception as e:
    ...
    def _generate_xoauth2_string(username: str, access_token: str) -> str:
        """Generate base64-encoded XOAUTH2 auth string."""
...
def main() -> None:
    ...
    try:
        ...
        # Ask whether this should be a Credit Memo (Kreditnota)
        credit_memo = Credit_memo()

        print("\n" + "="*60)
        if credit_memo:
            print("ST_FAKTURA - CREDIT MEMO (KREDITNOTA) CREATION")
        else:
        ...
        confirm_prompt = "\nDo you want to generate this credit memo? (y/N): " if credit_memo else "\nDo you want to generate this invoice? (y/N): "
        ...
        # Step 4: Generate invoice / credit memo
        print("\nStep 3: Generating Credit Memo..." if credit_memo else "\nStep 3: Generating Invoice...")
        pdf_path = invoice_manager.generate_invoice(selected_customer, selected_tasks, hourly_rate, credit_memo=credit_memo)
        
        if not pdf_path:
            print("❌ Failed to generate invoice PDF")
            sys.exit(1)
        
        print(f"✅ Credit Memo PDF generated: {pdf_path}" if credit_memo else f"✅ Invoice PDF generated: {pdf_path}")
        
        # Step 5: Send email (optional)
        if selected_customer['email']:
            email_prompt = (f"\nSend credit memo to {selected_customer['email']}? (y/N): " if credit_memo 
                            else f"\nSend invoice to {selected_customer['email']}? (y/N): ")
            send_email = input(email_prompt).strip().lower()
            ...
            if send_email == 'y':
                ...
                if invoice_manager.send_invoice_email(
                    ...
                    sent_label = "Credit memo" if credit_memo else "Invoice"
                    print(f"✅ {sent_label} sent to {selected_customer['email']}")
                else:
