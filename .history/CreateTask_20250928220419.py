...
This script allows users to create new tasks by selecting     def get_task_types(self) -> List[str]:
        """
        Retrieve all task types from the task types spreadsheet
        ...
        try:
            ...
            # Use the specific sheet gid URL for task types
            tasktype_url = f"https://docs.google.com/spreadsheets/d/{self.spreadsheet_id}/edit?gid={self.tasktype_gid}"
            tasktype_data = self.sheets_client.read_sheet(self.spreadsheet_id, TASKTYPE_SHEET_RANGE)and task types,
then storing them in the Google spreadsheet.

Customer data from: https://docs.google.com/spreadsh                self.sheets_client.write_sheet(
                    self.spreadsheet_id,
                    "A1:E1",
                    [headers]
                )d/170onDFFCveCzV6Q9F1_IhsG2LBRcw5MYxJbyocVJmq0/edit?gid=0#gid=0
...
# Task types sheet (gid=288943747) - Using column A for task types
TASKTYPE_SHEET_RANGE = "A:A"

# Tasks sheet (gid=1276274497) - Using columns A:E for task data
...
class TaskManager:
    ...
    def __init__(self, sheets_client: GoogleSheetsClient):
        ...
        # Different sheet gids within the same spreadsheet
        self.customers_gid = "0"  # Kunder sheet
        self.tasks_gid = "1276274497"  # Tasks sheet  
        self.tasktype_gid = "288943747"  # Tasktype sheet
    ...
    def add_new_task_type(self, new_task_type: str) -> bool:
        ...
        try:
            ...
            # Add the new task type to the spreadsheet
            # We'll append it to the Tasktype column
            self.sheets_client.append_to_sheet(
                self.spreadsheet_id,
                TASKTYPE_SHEET_RANGE,
                [[new_task_type]]
            )
    ...
    def setup_tasktype_spreadsheet_headers(self) -> None:
        """
        Set up the task types spreadsheet headers if they don't exist
        """
        try:
            logger.info("Checking task types spreadsheet headers")
            tasktype_data = self.sheets_client.read_sheet(self.spreadsheet_id, TASKTYPE_SHEET_RANGE)
            
            # If no data, add a header
            if not tasktype_data:
                logger.info("Setting up task types spreadsheet headers")
                self.sheets_client.write_sheet(
                    self.spreadsheet_id,
                    "A1",
                    [["Tasktype"]]
                )
                logger.info("Task types headers added successfully")
                
        except Exception as e:
            logger.error(f"Failed to setup task types headers: {e}")
...
def main() -> None:
    ...
    try:
        ...
        task_manager.setup_tasktype_spreadsheet_headers()
        ...
        if confirm == 'y':
            ...
            if task_manager.add_task(task_data):
                ...
                print(f"Description: {task_description[:50]}{'...' if len(task_description) > 50 else ''}")
                print(f"Time: {task_time} minutes")
                logger.info(f"Task creation completed for customer: {selected_customer['name']} with task type: {selected_tasktype}")
            else:
