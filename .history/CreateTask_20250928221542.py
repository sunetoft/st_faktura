...
# Tasks sheet (gid=1276274497) - Using gid directly since sheet name is unknown
...
TASKS_SHEET_RANGE = "A:E"  # Will use gid-based approach
...
class TaskManager:
    ...
    def __init__(self, sheets_client: GoogleSheetsClient):
        ...
        # All sheets are in the same spreadsheet, just different tabs/gids
        self.spreadsheet_id = SPREADSHEET_ID
        
        # Create separate configs for each sheet gid
        self.customer_config = SheetsConfig(
            sheet_url=CUSTOMER_SHEET_URL,
            auth_method="service_account"
        )
        self.tasktype_config = SheetsConfig(
            sheet_url=TASKTYPE_SHEET_URL,
            auth_method="service_account"
        )
        self.tasks_config = SheetsConfig(
            sheet_url=TASKS_SHEET_URL,
            auth_method="service_account"
        )
    ...
    def get_customers(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # Use customer sheet with gid=0
            customer_client = GoogleSheetsClient(self.customer_config)
            customers_data = customer_client.read_sheet(self.customer_config.spreadsheet_id, "A:H")
    ...
    def get_task_types(self) -> List[str]:
        ...
        try:
            ...
            # Use tasktype sheet with gid=288943747
            tasktype_client = GoogleSheetsClient(self.tasktype_config)
            tasktype_data = tasktype_client.read_sheet(self.tasktype_config.spreadsheet_id, "A:A")
    ...
    def add_task(self, task_data: Dict[str, str]) -> bool:
        ...
        try:
            ...
            # Use tasks sheet with gid=1276274497
            tasks_client = GoogleSheetsClient(self.tasks_config)
            tasks_client.append_to_sheet(
                self.tasks_config.spreadsheet_id,
                "A:E",
                [task_row]
            )
    ...
    def setup_tasks_spreadsheet_headers(self) -> None:
        ...
        try:
            ...
            # Use tasks sheet with gid=1276274497
            tasks_client = GoogleSheetsClient(self.tasks_config)
            tasks_data = tasks_client.read_sheet(self.tasks_config.spreadsheet_id, "A:E")
